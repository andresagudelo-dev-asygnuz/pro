...
| G5 | QA | **En curso — Fase 1** | 2026-05-01 | — | Test plans formales por HU (`tasks/test-plans/HU-001.md`…`HU-006.md`) con casos GIVEN/WHEN/THEN + cobertura mapeada a `lib/tournaments/*`. Unit coverage `lib/tournaments` subida de **46%→88%** (api.ts 91%, matches.ts 94%, registrations.ts 82%). 188/188 tests PASS. Scripts SQL de verificación RLS+triggers en `tasks/test-plans/sql/` (ejecutar sobre Supabase local por el fundador: el sandbox no pudo levantar la imagen por rate-limit de `public.ecr.aws`). |
