...
export function SolutionSection() {
  ...
  return (
    <section ref={sectionRef} className="relative py-32 mesh-gradient overflow-hidden">
      <div className="absolute inset-0 bg-[url('/hero-bg.png')] bg-fixed bg-cover bg-center opacity-30" />
            ...
            return (
              <div key={index} className="solution-card group relative">
                <GlassContainer className="h-full flex flex-col items-center text-center p-12 border-white/10 bg-white/5 backdrop-blur-xl hover:border-white/30 transition-all duration-500 overflow-hidden rounded-[2.5rem]">
                  <div
                    className={`absolute -top-24 -right-24 w-48 h-48 bg-gradient-to-br ${item.color} opacity-0 group-hover:opacity-20 blur-[60px] transition-opacity duration-700`}
                  />
                  <div
                    className={`absolute -bottom-24 -left-24 w-48 h-48 bg-gradient-to-tr ${item.color} opacity-0 group-hover:opacity-10 blur-[60px] transition-opacity duration-700`}
                  />

                  <div className="relative mb-10">
                    <div
                      className={`text-9xl font-black bg-gradient-to-b ${item.color} bg-clip-text text-transparent opacity-20 absolute -top-12 left-1/2 -translate-x-1/2 select-none tracking-tighter`}
                    >
                      {item.number}
                    </div>
                  ...
                  <p className="text-white/70 text-lg leading-relaxed font-bold">{item.description}</p>
