      ...
      <div className="container mx-auto relative z-10 px-6">
        ...
        <div ref={containerRef} className="grid grid-cols-1 md:grid-cols-3 gap-8 challenge-cards-container">
          {challenges.map((item, index) => (
            <GlassContainer
              key={index}
              className="challenge-card flex flex-col items-center text-center p-12 bg-white/40 dark:bg-white/5 border-zinc-200 dark:border-white/10 hover:border-brand-primary/50 transition-all duration-700 hover:-translate-y-4 group shadow-xl hover:shadow-brand-primary/10"
            >
              ...
              <h3 className="text-2xl font-black mb-6 text-zinc-900 dark:text-white tracking-tight uppercase italic">
                {item.title}
              </h3>
              <p className="text-zinc-600 dark:text-zinc-400 leading-relaxed font-bold">{item.description}</p>
