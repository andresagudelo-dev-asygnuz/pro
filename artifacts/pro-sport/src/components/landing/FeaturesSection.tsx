...
import { Calendar, MessageSquare, BarChart3, MapPin, ShieldCheck, Share2 } from "lucide-react";

gsap.registerPlugin(ScrollTrigger);

const features = [
  { icon: Calendar, title: "Reserva de Canchas", description: "Encuentra y reserva canchas sintéticas en segundos con disponibilidad real." },
  { icon: MessageSquare, title: "Arma tu Equipo", description: "Chat integrado para coordinar el 5-a-side o el 11-a-side sin salir de la app." },
  { icon: BarChart3, title: "Tablero de Goleadores", description: "Registra tus estadísticas, goles y asistencias para competir por el Pichichi local." },
  { icon: MapPin, title: "Radar de Partidos", description: "Localiza retos abiertos y partidos que necesitan jugadores cerca de ti." },
  { icon: ShieldCheck, title: "Fair Play", description: "Sistema de reputación para garantizar que cada partido sea competitivo y limpio." },
  { icon: Share2, title: "MVP de la Fecha", description: "Comparte tus mejores jugadas, trofeos y estadísticas en tus redes sociales." },
];
          ...
          {features.map((feature, index) => (
            <div key={index} className="group relative">
              ...
              <GlassContainer className="h-full border-zinc-200 dark:border-white/10 bg-white/40 dark:bg-white/5 p-10 transition-all duration-700 hover:-translate-y-4 hover:scale-[1.05] hover:border-brand-primary/40 cursor-default shadow-sm hover:shadow-2xl flex flex-col items-center text-center">
                ...
                <p className="text-zinc-600 dark:text-zinc-400 leading-relaxed font-medium">{feature.description}</p>
              </GlassContainer>
            </div>
          ))}
        </div>
