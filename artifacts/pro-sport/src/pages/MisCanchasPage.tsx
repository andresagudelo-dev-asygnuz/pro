...
export default function MisCanchasPage() {
  ...
  async function toggleActive(cancha: Cancha) {
    ...
    if (error) {
      ...
      setCanchas((prev) =>
        prev.map((c) => (c.id === cancha.id ? { ...c, is_active: !c.is_active } : c)),
      );
      ...
      <AppLayout>
        <div className="container py-8 max-w-lg mx-auto">
          <div className="bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-700 rounded-xl p-6 text-center">
            ...
            <Button onClick={() => setLocation("/perfil")}>Ir a mi perfil</Button>
          </div>
        </div>
      </AppLayout>
    );
      ...
      <div className="container py-8 max-w-4xl mx-auto space-y-6">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-0.5">Gestioná las canchas que administrás.</p>
          </div>
          <Button asChild>
            <Link href="/canchas/nueva">
              <Plus className="size-4 mr-1" /> Nueva cancha
            </Link>
          </Button>
        </div>
        ...
        {loading ? (
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            {canchas.map((c) => (
              <div key={c.id} className="border rounded-xl p-5 bg-white dark:bg-zinc-900 shadow-sm space-y-4">
                ...
                <div className="text-sm">
                  <span className="font-semibold text-brand-primary">${c.price_per_hour.toLocaleString("es-CO")}/h</span>
                  {c.discount_percent > 0 && (
                    <span className="ml-2 text-xs text-green-600">-{c.discount_percent}% dto.</span>
                  )}
                </div>
                ...
                <div className="flex gap-2">
                  ...
                  <Button variant="outline" size="sm" className="flex-1" asChild>
                    <Link href={`/canchas/${c.id}`}>Ver cancha</Link>
                  </Button>
                </div>
              </div>
            ))}
