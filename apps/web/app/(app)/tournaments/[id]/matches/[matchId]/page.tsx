...
export default function MatchResultPage({ params }: PageProps) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const [{ data: t }, { data: m }, { data: regs }, { data: evs }] =
        await Promise.all([
      ...
      setIsOwner(!!userAuth.user && userAuth.user.id === tRow.owner_id);
  ...
  if (loading) {
    return <div className="container py-8 max-w-3xl mx-auto">Cargando…</div>;
  }
  if (!tournament || !match) {
    return (
      ...
      <div className="flex items-center justify-between gap-4 flex-wrap">
        <div>
          <h1 className="text-2xl font-bold tracking-tight">
            {regLabel(match.home_registration_id)} vs{" "}
            {regLabel(match.away_registration_id)}
          </h1>
        ...
        <Button variant="outline">
          <Link href={`/tournaments/${id}/matches`}>Volver a partidos</Link>
        </Button>
      </div>
      ...
      {isOwner ? (
        ...
        >
          ...
          <div className="flex items-end gap-3">
            <div>
              <Label htmlFor="home-score">
                {regLabel(match.home_registration_id)}
              </Label>
              <Input
                id="home-score"
                type="number"
          ...
          <p className="text-xs text-muted-foreground">
            Al guardar como <code>finalizado</code> la tabla de posiciones se
      ...
      <section className="border rounded-lg p-4 space-y-4">
            ...
            <li key={ev.id} className="flex items-center gap-2">
              ...
              {ev.team_side && (
                <span className="text-xs text-muted-foreground">
                  ({ev.team_side === "home" ? "local" : "visitante"})
                </span>
              )}
              {ev.notes && (
                <span className="text-xs text-muted-foreground">
        ...
        {isOwner && (
          <form
            ...
            className="grid grid-cols-1 sm:grid-cols-5 gap-2 border-t pt-4"
          >
            <select
            ...
            >
              {(Object.keys(eventLabels) as MatchEventType[]).map((k) => (
