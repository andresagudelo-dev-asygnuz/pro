...
export default function NewMatchPage({ params }: PageProps) {
  const { id } = use(params);
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (tRow.owner_id !== userAuth.user.id) {
      ...
      if (!active) return;
      setRegistrations(
        ((regs ?? []) as RegistrationRow[]).filter(
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setError(null);
    if (!home || !away) {
      setError("Seleccioná local y visitante.");
      return;
    }
    setSubmitting(true);
  ...
  if (loading) {
    return <div className="container py-8 max-w-2xl mx-auto">Cargando…</div>;
  }
  if (!tournament) {
    return (
  ...
  const regLabel = (r: RegistrationRow) =>
    r.team_id
      ? `Equipo ${r.team_id.slice(0, 8)}`
      : r.user_id
        ? `Jugador ${r.user_id.slice(0, 8)}`
        : r.id.slice(0, 8);
    ...
    <div className="container py-8 max-w-2xl mx-auto space-y-6">
      <div>
        <h1 className="text-3xl font-bold tracking-tight">
          Nuevo partido — {tournament.name}
        </h1>
        <p className="text-muted-foreground mt-1 text-sm">
          Agregá un encuentro al fixture. Podés cargar el resultado después.
        </p>
      </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex gap-2">
            ...
            <Button variant="outline" type="button">
              <Link href={`/tournaments/${id}/matches`}>Cancelar</Link>
            </Button>
          </div>
        </form>
      )}
